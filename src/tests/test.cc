...
TEST(CreateTest, ParametrizedConstructorFailure) {
  EXPECT_THROW(S21Matrix example(-1, 1), std::invalid_argument);
}
...
TEST(TestPrivateAccess, MutatorsInvailid) {
  int rows = 3, cols = 3;
  S21Matrix matrix(rows, cols);
  EXPECT_THROW(matrix.set_cols(-4), std::invalid_argument);
  EXPECT_THROW(matrix.set_rows(-4), std::invalid_argument);
}
